...
type ApiFinancialYear = { year: string; revenue: number; ebit: number; net_income: number; operating_cash_flow: number; capex: number; total_debt: number; cash: number };
type TickerApiResponse = { success: true; symbol: string; name: string; currentPrice: number; beta: number; sharesOutstanding: number; financials: ApiFinancialYear[] } | { success: false; error: string };
const quickTickers = ["MSFT", "NVDA", "AAPL", "TSLA", "AMZN", "GOOGL"];
  ...
  const fetchTicker = async (ticker: string) => {
    ...
    try {
      ...
      setCompany({
        ...
        financials: payload.financials.map((row) => ({
          year: row.year,
          revenue: row.revenue / 1_000_000,
          ebit: row.ebit / 1_000_000,
          operatingCashFlow: row.operating_cash_flow / 1_000_000,
          capex: row.capex / 1_000_000,
          totalDebt: row.total_debt / 1_000_000,
          cash: row.cash / 1_000_000,
          sharesOutstanding: payload.sharesOutstanding / 1_000_000,
          netIncome: row.net_income / 1_000_000,
          equity: Math.max((row.revenue * 0.42) / 1_000_000, 1),
          ebitda: Math.max((row.ebit * 1.15) / 1_000_000, 1),
          interestExpense: Math.max((row.total_debt * 0.035) / 1_000_000, 1),
          investedCapital: Math.max(((row.total_debt + row.revenue * 0.42 - row.cash) / 1_000_000), 1),
          marketPrice: payload.currentPrice,
        })),
        catalysts: [
          `${payload.symbol} fundamentals normalized from Yahoo Finance server-side data`,
      ...
      setUploadMessage(`Live fundamentals loaded for ${payload.symbol}.`);
    } catch (error) {
