...
Bay Oceania C&T Ltd. requires a more disciplined analytics operating model for commercial construction and government procurement tender management. This standalone repository demonstrates a complete analytics case study for quantifying tender pipeline value, prioritizing deal focus, and validating process efficiency gains through Lean Six Sigma and Agile delivery practices.
...
- **$150K+ captured tender value threshold** assessed through Monte Carlo simulation.
- **20%+ proposal turnaround reduction** validated through pre/post DMAIC workflow analysis.
- **15+ secured or high-priority opportunities** identified for pursuit focus and resource allocation.
- **Risk-adjusted revenue visibility** across sector, stage, client, probability, margin, and risk rating.

## Business Problem Statement

Bay Oceania C&T Ltd. historically managed commercial construction and public-sector tender pursuits through fragmented spreadsheet trackers, informal status updates, and manual reporting. During high-pressure government procurement rounds, this created four recurring management issues:

1. **Delayed decision cycles** because opportunity value, win probability, and proposal readiness were not consolidated into a single analytical view.
2. **Proposal turnaround bottlenecks** caused by sequential handoffs across estimating, technical, commercial, compliance, and review teams.
...
- Sprint reviews: review gates before submission.
...
- **Python:** Synthetic data generation, Monte Carlo simulation, statistical hypothesis testing, and summary logging.
...
- **SQL:** PostgreSQL/SQLite-compatible pipeline analysis queries for stage value, weighted revenue, bottleneck analysis, and high-ROI opportunity targeting.
- **Power BI / DAX:** dashboard measures for total pipeline value, weighted expected revenue, win rate, and turnaround reduction.
...
SQL Reporting Layer      Monte Carlo + t-test Outputs
        ...
        +----------+-----------+
                   |
                   v
dax_measures.dax ---> Power BI Dashboard
```
...
| `pipeline_analytics.py` | Runs Monte Carlo simulation and statistical testing for analytics decision support. |
...
- Probability of crossing the $150K captured value threshold.
...
The case study moves leadership reporting beyond raw pipeline value by applying tender-level win probability to contract value. This enables teams to distinguish optimistic pipeline volume from realistic expected revenue.
...
The high-ROI SQL query identifies opportunities above $100K in expected contract value, above 50% win probability, and above 18% margin. These opportunities should receive priority pursuit focus, senior estimator capacity, and proactive client engagement.
...
This repository demonstrates a practical analytics foundation that can be extended into CRM integration, estimator capacity planning, bid/no-bid scoring, supplier risk analysis, and Power BI scorecards.
...
4. **High-ROI Targets:** opportunity list with margin, value, probability, and risk.
5. **Monte Carlo Forecast:** Confidence interval for captured revenue and probability of meeting the $150K threshold.
