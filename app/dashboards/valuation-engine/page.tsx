...
export default function ValuationEnginePage() {
  ...
  const loadTicker = async (ticker: string) => {
    ...
    try {
      ...
      setCompany({ ticker: payload.symbol, name: payload.name, currency: payload.currency ?? "USD", marketPrice: payload.currentPrice, financials: normalizedFinancials, catalysts: [`Live Yahoo chart price resolved for ${payload.symbol}`, "Fundamentals normalized for institutional DCF modeling", "Sensitivity matrix and PDF memo refresh automatically"], sentiment: [`${payload.symbol} loaded through the serverless multi-tier ticker engine`, `Estimated beta ${payload.beta.toFixed(2)} informs risk review`, "Upload CSV remains available for management-case overrides"] });
